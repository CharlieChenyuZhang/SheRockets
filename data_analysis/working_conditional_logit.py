#!/usr/bin/env python3
"""
Working Conditional Logit Analysis
=================================

A working implementation that properly handles collinearity
"""
...
def run_working_conditional_logit():
    """
    Run working conditional logit analysis
    """
    print("Loading data...")
...
def convert_to_choice_format(df):
    ...
    for idx, row in df.iterrows():
        for task in range(1, 9):
            ...
            # Add attribute differences (A - B) - only one dummy per attribute to avoid collinearity
            for attr in ['Tutor', 'Color_palette', 'Pricing', 'Message_success_', 
                ...
                # Create single dummy variable for each attribute (avoiding collinearity)
                record.update(create_single_attribute_dummy(attr, a_val, b_val))
            
            choice_records.append(record)
    
    return pd.DataFrame(choice_records)

def create_single_attribute_dummy(attr, a_val, b_val):
    """
    Create single dummy variable for an attribute to avoid collinearity
    """
    dummies = {}
    
    if attr == 'Tutor':
        # Female tutor dummy (1 if A is female, 0 if B is female, -1 if both same)
        dummies['female_tutor_diff'] = (1 if a_val == 'Female AI tutor' else 0) - (1 if b_val == 'Female AI tutor' else 0)
    
    elif attr == 'Color_palette':
        # Friendly colors dummy
        dummies['friendly_colors_diff'] = (1 if 'Friendly & warm' in a_val else 0) - (1 if 'Friendly & warm' in b_val else 0)
    
    elif attr == 'Pricing':
        # Create pricing dummies (exclude one level to avoid collinearity)
        ...
        # Exclude pricing_12.99 to avoid collinearity
    
    elif attr == 'Message_success_':
        # Growth message dummy
...
def create_results_table(results_dict):
    ...
    print("\n" + "="*120)
    print("WORKING CONDITIONAL LOGIT ANALYSIS RESULTS")
    print("="*120)
    ...
    print("Note: Only one level per attribute is shown to avoid collinearity")
    
    # Save to CSV with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f'/Users/charlie/github.com/hai/SheRockets/data_analysis/working_conditional_logit_results_{timestamp}.csv'
...
def main():
    ...
    Main function
    """
    print("Starting WORKING CONDITIONAL LOGIT Analysis")
    print("="*50)
    
    # Run analysis
    results, output_file = run_working_conditional_logit()
    
    print("\nWORKING CONDITIONAL LOGIT Analysis completed!")
    print(f"Results saved to: {output_file}")
